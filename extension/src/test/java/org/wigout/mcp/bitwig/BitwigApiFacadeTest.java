...
public class BitwigApiFacadeTest {
    ...
    void setUp() {
        ...
        // hasNext/hasPrevious are init-marked for the post-write DirectParameter
        // refresh bounce-direction pick; stub so construction doesn't NPE.
        lenient().when(mockCursorDevice.hasNext()).thenReturn(mock(com.bitwig.extension.controller.api.BooleanValue.class));
        lenient().when(mockCursorDevice.hasPrevious()).thenReturn(mock(com.bitwig.extension.controller.api.BooleanValue.class));
        lenient().when(mockCursorTrack.hasNext()).thenReturn(mock(com.bitwig.extension.controller.api.BooleanValue.class));
        lenient().when(mockCursorTrack.hasPrevious()).thenReturn(mock(com.bitwig.extension.controller.api.BooleanValue.class));
        ...
        // Bridge-root creation stubs (Task: full-API bridge cycle 1)
        when(mockHost.getProject()).thenReturn(mock(Project.class));
        when(mockHost.createArranger()).thenReturn(mock(Arranger.class));
        when(mockHost.createMixer()).thenReturn(mock(Mixer.class));
        when(mockHost.createGroove()).thenReturn(mock(Groove.class));
        PopupBrowser mockPopupBrowser = mock(PopupBrowser.class);
        when(mockHost.createPopupBrowser()).thenReturn(mockPopupBrowser);
        // Task 10 (Cycle 2): each browser column is a distinct mock, each
        // returning a distinct mock item bank from createItemBank (init-only).
        stubBrowserColumn(mockPopupBrowser::smartCollectionColumn);
        stubBrowserColumn(mockPopupBrowser::locationColumn);
        stubBrowserColumn(mockPopupBrowser::deviceColumn);
        stubBrowserColumn(mockPopupBrowser::categoryColumn);
        stubBrowserColumn(mockPopupBrowser::tagColumn);
        stubBrowserColumn(mockPopupBrowser::deviceTypeColumn);
        stubBrowserColumn(mockPopupBrowser::fileTypeColumn);
        stubBrowserColumn(mockPopupBrowser::creatorColumn);
        BrowserResultsColumn mockResultsColumn = mock(BrowserResultsColumn.class);
        when(mockPopupBrowser.resultsColumn()).thenReturn(mockResultsColumn);
        when(mockResultsColumn.createItemBank(32)).thenReturn(mock(BrowserResultsItemBank.class));
        when(mockCursorTrack.createLauncherCursorClip(128, 128)).thenReturn(mock(PinnableCursorClip.class));

        // Task 6 (Cycle 2): cursor-anchored depth domains — drum pads, layers, slot chain.
        DrumPadBank mockDrumPadBank = mock(DrumPadBank.class);
        when(mockCursorDevice.createDrumPadBank(16)).thenReturn(mockDrumPadBank);
        DrumPad mockDrumPad = mock(DrumPad.class);
        when(mockDrumPadBank.getItemAt(anyInt())).thenReturn(mockDrumPad);
        when(mockDrumPad.createDeviceBank(4)).thenReturn(mock(DeviceBank.class));

        DeviceLayerBank mockLayerBank = mock(DeviceLayerBank.class);
        when(mockCursorDevice.createLayerBank(8)).thenReturn(mockLayerBank);
        DeviceLayer mockDeviceLayer = mock(DeviceLayer.class);
        when(mockLayerBank.getItemAt(anyInt())).thenReturn(mockDeviceLayer);
        when(mockDeviceLayer.createDeviceBank(4)).thenReturn(mock(DeviceBank.class));

        DeviceSlot mockDeviceSlot = mock(DeviceSlot.class);
        when(mockCursorDevice.getCursorSlot()).thenReturn(mockDeviceSlot);
        when(mockDeviceSlot.createDeviceBank(8)).thenReturn(mock(DeviceBank.class));
    ...
    @AfterEach
    void tearDown() {
        // The constructor installs a real event-thread runner on the
        // process-wide SubscribeSettle static (capturing this test's mock
        // host) on every run — reset it so it doesn't leak into other test
        // classes that assume inline (null-runner) bridge registration.
        SubscribeSettle.install(null);
    }

    /**
     * Task 10 (Cycle 2): stubs one popup-browser filter-column accessor to
     * return a distinct mock column, which in turn returns a distinct mock
     * item bank from createItemBank(16) — mirrors how earlier tasks stubbed
     * other init-only creations (drum pad/layer/slot device banks above).
     */
    private BrowserFilterColumn stubBrowserColumn(Supplier<BrowserFilterColumn> accessor) {
        BrowserFilterColumn column = mock(BrowserFilterColumn.class);
        when(accessor.get()).thenReturn(column);
        when(column.createItemBank(16)).thenReturn(mock(BrowserFilterItemBank.class));
        return column;
    }
    ...
    void testSetSelectedDeviceParameter_Success() {
        ...
        // Assert
        verify(mockValueSetter).setImmediately(value);
    ...
    void testSetSelectedDeviceParameter_BoundaryValues() {
        ...
        verify(mockValueSetter).setImmediately(0.0);

        // Test maximum boundary values
        bitwigApiFacade.setSelectedDeviceParameter(7, 1.0);
        verify(mockValueSetter).setImmediately(1.0);
    ...
    @Test
    void setSelectedDeviceParameter_usesSetImmediately_notSet() throws Exception {
        // Mock device exists
        com.bitwig.extension.controller.api.BooleanValue mockExists = mock(com.bitwig.extension.controller.api.BooleanValue.class);
        when(mockExists.get()).thenReturn(true);
        when(mockCursorDevice.exists()).thenReturn(mockExists);

        com.bitwig.extension.controller.api.SettableRangedValue value =
            mock(com.bitwig.extension.controller.api.SettableRangedValue.class);
        when(mockRemoteControl.value()).thenReturn(value);
        when(mockParameterBank.getParameter(3)).thenReturn(mockRemoteControl);

        bitwigApiFacade.setSelectedDeviceParameter(3, 0.3);

        verify(value).setImmediately(0.3);
        verify(value, never()).set(anyDouble());
    }

    @Test
    void getSelectedDeviceParameterValue_returnsCachedValue() throws Exception {
        // Mock device exists
        com.bitwig.extension.controller.api.BooleanValue mockExists = mock(com.bitwig.extension.controller.api.BooleanValue.class);
        when(mockExists.get()).thenReturn(true);
        when(mockCursorDevice.exists()).thenReturn(mockExists);
        ...
        com.bitwig.extension.controller.api.SettableRangedValue value =
            mock(com.bitwig.extension.controller.api.SettableRangedValue.class);
        when(mockRemoteControl.value()).thenReturn(value);
        when(value.get()).thenReturn(0.42);
        when(mockParameterBank.getParameter(3)).thenReturn(mockRemoteControl);

        assertEquals(0.42, bitwigApiFacade.getSelectedDeviceParameterValue(3), 1e-9);
    }
    ...
    // ---- Task: track construction ----

    /** Stubs a Track mock that exists and reports the given name. */
    private Track mockExistingTrack(String name) {
    ...
    @Test
    void testGetExistingTrackNamesSkipsNonExisting() {
        when(mockTrackBank.getSizeOfBank()).thenReturn(3);
        Track drums = mockExistingTrack("Drums");
        Track bass = mockExistingTrack("Bass");
        Track ghost = mock(Track.class);
        BooleanValue ghostExists = mock(BooleanValue.class);
        when(ghostExists.get()).thenReturn(false);
        when(ghost.exists()).thenReturn(ghostExists);
        when(mockTrackBank.getItemAt(0)).thenReturn(drums);
        when(mockTrackBank.getItemAt(1)).thenReturn(ghost);
        when(mockTrackBank.getItemAt(2)).thenReturn(bass);

        assertEquals(List.of("Drums", "Bass"), bitwigApiFacade.getExistingTrackNames());
    }

    @Test
    void testCreateTrackDispatchesByType() throws Exception {
        bitwigApiFacade.createTrack("instrument", -1);
        verify(mockApplication).createInstrumentTrack(-1);

        bitwigApiFacade.createTrack("audio", 2);
        verify(mockApplication).createAudioTrack(2);

        bitwigApiFacade.createTrack("effect", 0);
        verify(mockApplication).createEffectTrack(0);
    }

    @Test
    void testCreateTrackRejectsUnknownType() {
        BitwigApiException e = assertThrows(BitwigApiException.class,
            () -> bitwigApiFacade.createTrack("midi", -1));
        assertEquals(ErrorCode.INVALID_PARAMETER, e.getErrorCode());
        verify(mockApplication, never()).createInstrumentTrack(anyInt());
        verify(mockApplication, never()).createAudioTrack(anyInt());
        verify(mockApplication, never()).createEffectTrack(anyInt());
    }

    @Test
    void testRenameTrackByIndex() throws Exception {
        when(mockTrackBank.getSizeOfBank()).thenReturn(2);
        Track track = mockExistingTrack("Old");
        when(mockTrackBank.getItemAt(0)).thenReturn(track);

        bitwigApiFacade.renameTrackByIndex(0, "New Name");

        verify(track.name()).set("New Name");
    }

    @Test
    void testRenameTrackByIndexInvalidIndexThrows() {
        when(mockTrackBank.getSizeOfBank()).thenReturn(2);
        assertThrows(BitwigApiException.class,
            () -> bitwigApiFacade.renameTrackByIndex(5, "X"));
    }

    @Test
    void testDeleteTrackByIndex() throws Exception {
        when(mockTrackBank.getSizeOfBank()).thenReturn(2);
        Track track = mockExistingTrack("Doomed");
        when(mockTrackBank.getItemAt(1)).thenReturn(track);

        bitwigApiFacade.deleteTrackByIndex(1);

        verify(track).deleteObject();
    }

    @Test
    void testInsertBitwigDeviceAtEndOfChain() throws Exception {
        when(mockTrackBank.getSizeOfBank()).thenReturn(1);
        Track track = mockExistingTrack("Synths");
        InsertionPoint insertionPoint = mock(InsertionPoint.class);
        when(track.endOfDeviceChainInsertionPoint()).thenReturn(insertionPoint);
        when(mockTrackBank.getItemAt(0)).thenReturn(track);

        UUID polymer = UUID.fromString("8f58138b-03aa-4e9d-83bd-a038c99a4ed5");
        bitwigApiFacade.insertBitwigDeviceAtEndOfChain(0, polymer);

        verify(insertionPoint).insertBitwigDevice(polymer);
    }

    @Test
    void testInsertFileAtEndOfChain() throws Exception {
        when(mockTrackBank.getSizeOfBank()).thenReturn(1);
        Track track = mockExistingTrack("Synths");
        InsertionPoint insertionPoint = mock(InsertionPoint.class);
        when(track.endOfDeviceChainInsertionPoint()).thenReturn(insertionPoint);
        ...
        bitwigApiFacade.insertFileAtEndOfChain(0, "C:/presets/fat-bass.bwpreset");

        verify(insertionPoint).insertFile("C:/presets/fat-bass.bwpreset");
    }

    // ---- DirectParameter bridging: init-only observer registration ----
    // No display observer: setObservedParameterIds halts the extension on
    // Bitwig 6.1b4 in every invocation context (live finding #45).

    @Test
    void testDirectParameterObserversAreRegisteredAtInit() {
        verify(mockCursorDevice).addDirectParameterIdObserver(any());
        verify(mockCursorDevice).addDirectParameterNameObserver(eq(64), any());
        verify(mockCursorDevice).addDirectParameterNormalizedValueObserver(any());
        verify(mockCursorDevice, never()).addDirectParameterValueDisplayObserver(anyInt(), any());
    }

    @Test
    void testIdObserverFeedsDirectParameterCacheOnGraph() {
        ArgumentCaptor<StringArrayValueChangedCallback> idCaptor =
            ArgumentCaptor.forClass(StringArrayValueChangedCallback.class);
        verify(mockCursorDevice).addDirectParameterIdObserver(idCaptor.capture());

        idCaptor.getValue().valueChanged(new String[] {"p1", "p2"});

        assertEquals(2, bitwigApiFacade.getBridgeGraph().directParameterCache().size());
    }

    // ---- Post-write DirectParameter refresh (bounce) — fast guard paths ----
    // The async bounce-and-settle success path is covered by the live gate.

    @Test
    void testRefreshReturnsNullWhenIdNotObserved() {
        // Empty cache — no bounce should be scheduled for an unknown id.
        assertNull(bitwigApiFacade.refreshDirectParametersAfterWrite("CONTENTS/PID9"));
        verify(mockHost, never()).scheduleTask(any(), anyLong());
    }

    @Test
    void testRefreshReturnsNullWhenNoSiblingToBounceTo() {
        // Populate the cache so the id-guard passes, then leave every
        // hasNext/hasPrevious false (default mock BooleanValue.get()) — there is
        // no device or track sibling, so the bounce can't run and we honestly
        // report failure without scheduling anything.
        ArgumentCaptor<StringArrayValueChangedCallback> idCaptor =
            ArgumentCaptor.forClass(StringArrayValueChangedCallback.class);
        verify(mockCursorDevice).addDirectParameterIdObserver(idCaptor.capture());
        idCaptor.getValue().valueChanged(new String[] {"CONTENTS/PID2"});

        assertNull(bitwigApiFacade.refreshDirectParametersAfterWrite("CONTENTS/PID2"));
        verify(mockHost, never()).scheduleTask(any(), anyLong());
    }

    @Test
    void testRefreshBouncesDeviceChainAndReturnsConfirmedValue() {
        var cache = bitwigApiFacade.getBridgeGraph().directParameterCache();
        com.bitwig.extension.controller.api.BooleanValue hasNext =
            mock(com.bitwig.extension.controller.api.BooleanValue.class);
        when(hasNext.get()).thenReturn(true);
        when(mockCursorDevice.hasNext()).thenReturn(hasNext);
        // Run scheduled tasks synchronously AND simulate Bitwig re-firing the id
        // observer after each selection change (advancing generation), so the
        // bounce settles and the poll returns the freshly observed value.
        doAnswer(inv -> {
            ((Runnable) inv.getArgument(0)).run();
            cache.onIds(new String[] {"CONTENTS/PID2"});
            cache.onValue("CONTENTS/PID2", 0.5005);
            return null;
        }).when(mockHost).scheduleTask(any(), anyLong());
        ...
        Double confirmed = bitwigApiFacade.refreshDirectParametersAfterWrite("CONTENTS/PID2");
        assertEquals(0.5005, confirmed, 1e-9);
        verify(mockCursorDevice).selectNext();   // away hop (device chain)
        verify(mockCursorDevice).selectPrevious(); // back hop
    }
}
