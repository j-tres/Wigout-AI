            ...
            (exchange, req) -> McpErrorHandler.executeWithErrorHandling(
                ...
                new McpErrorHandler.ToolOperation() {
                    ...
                    public Object execute() throws Exception {
                        ...
                        boolean verified = deviceController.setSelectedDeviceParameter(args.parameterIndex(), args.value());
                        Map<String, Object> data = new LinkedHashMap<>();
                        data.put("action", "parameter_set");
                        data.put("parameter_index", args.parameterIndex());
                        data.put("new_value", args.value());
                        data.put("verified", verified);
                        data.put("message", verified
                            ? "Parameter " + args.parameterIndex() + " set to " + args.value() + " (verified)."
                            : "Parameter " + args.parameterIndex() + " set to " + args.value() + " but could NOT be verified — the value did not reach the target (check the device's take-over mode or that the parameter is writable).");
                        return data;
                    }
                }
            );
                    ...
                    public Object execute() throws Exception {
                        ...
                        for (ParameterSettingResult result : results) {
                            ...
                            if ("success".equals(result.status())) {
                                ...
                                resultMap.put("verified", result.verified());
                            } else {
